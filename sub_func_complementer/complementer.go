...
import (
	...
	"data-platform-api-shop-creates-rmq-kube/config"
	"encoding/json"
...
func (c *SubFuncComplementer) ComplementHeader(input *dpfm_api_input_reader.SDC, subfuncSDC *SDC, l *logger.Logger) error {
	s := &SDC{}
	numRange, err := c.ComplementShop(input, l)
	if err != nil {
		return xerrors.Errorf("complement shop error: %w", err)
	}
...
func (c *SubFuncComplementer) ComplementShop(input *dpfm_api_input_reader.SDC, l *logger.Logger) (*NumberRange, error) {
	rows, err := c.db.Query(
		...
		WHERE (ServiceLabel, FieldNameWithNumberRange) = ( (?, 'Shop') );`, input.ServiceLabel,
	)
	if err != nil {
	...
	nr.LatestNumber++
	input.Header.Shop = nr.LatestNumber
	return &nr, nil
}
